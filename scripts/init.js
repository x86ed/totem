...
const initType = typeArg ? typeArg.split('=')[1] : (initFlag ? 'config' : null);
...
- **Areas of Expertise**: ${config.areasOfExpertise.join(', ')}
...
const templates = {
  config: {
    ...
    generate(config) {
      ...
      console.log(`🚀 Expertise: ${config.areasOfExpertise.join(', ')}`);
...
async function main() {
  ...
  if (!type) {
    ...
    const choices = { '0': 'config', '1': 'project', '2': 'domain', '3': 'component' };
    type = choices[choice];
  }
  ...
  try {
    const data = await templates[type].prompt();
    templates[type].generate(data);
